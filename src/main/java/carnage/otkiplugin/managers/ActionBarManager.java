    ...
    public class ActionBarManager {
        ...
        private void updatePlayerActionBar(Player player) {
            ...
            else if (isEtherioItem(mainHand)) {
                String abilityName = EtherioItem.getAbilityNameFromPlayer(player);
                double cooldown = Etherio.getCooldownRemaining(player, abilityName);
                player.sendActionBar(formatActionBar(abilityName, cooldown));
            }
        ...
        private boolean isEtherioItem(ItemStack item) {
            if (item == null || item.getType() != Material.ENDER_PEARL || item.getItemMeta() == null) return false;
            return item.getItemMeta().hasDisplayName() && ChatColor.stripColor(item.getItemMeta().getDisplayName()).equals("Etherio");
        }
